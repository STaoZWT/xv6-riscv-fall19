...
int bitD2_isset(char *array, int index) {
  index = index / 2;
  // why /2 then /8, not /16?
  // 
  char b = array[index/8];
...
void bitD2_change(char *array, int index) {
  index = index / 2;
  char b = array[index / 8];
  char m = (1 << index % 8);
  array[index/8] = (b ^ m);
...
bd_malloc(uint64 nbytes)
{
  ...
  bitD2_change(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    ...
    bitD2_change(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bitD2_change(bd_sizes[k].alloc, bi);
